...
export class StylePaginatorDirective implements AfterViewInit {
        ...
        for (let i = 0; i < nodeArray.length; i++) {
          if (nodeArray[i].nodeName === 'BUTTON') {
            if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
              ...
              this.ren.setStyle(nodeArray[i], 'background-color', '#3f51b5');
            }
          }
        }
      });
  ...
  /*   private buildPageNumbers() {
    ...
    for (let i = 0; i < this.numOfPages; i++) {
      if (i >= this._rangeStart && i <= this._rangeEnd) {
        const button = this.createButton(i, this.matPag.pageIndex);
        actionContainer.appendChild(button);
      } else if (i === this._rangeEnd + 1 && i < this.numOfPages - 1) {
        // Show ellipsis between buttons
        const ellipsisButton = this.createButton(
          this._pageGapTxt,
          this._rangeEnd
        );
        actionContainer.appendChild(ellipsisButton);
      }
    }
  } */
